...
export default class Highlighter extends Component {
  ...
  setColor(x){
    ...
    document.getElementById("OwnColor").style.background=toString(x.target.value)
  }
  setBackgroud=(e)=>{
      ...
      <>
       <div>
        <div className=" hideMe sm:text-5xl text-2xl font-josefins py-5 font-extrabold flex justify-center mt-5" id='code-share'>{this.state.code}</div>
         ...
         <div className=" w-full py-10 border-gray-50 rounded-xl flex justify-center">
          <div className=" w-5/6  h-4/5 bg-slate-50 mt-10 rounded-xl gradientShadow">
            <div className='w-full h-full border-red-500 rounded-xl bg-black'>
              <div className="container-plate w-full px-5 py-2 flex justify-around border-b-orange-500">
                ...
                <Select onChange={this.setThemes.bind(this)} options={themes} />
                </div>
                <div>
                <input type="checkbox" name="lineNumber" id="lineNumber" onChange={this.handleLineNum.bind(this)} className="isChecked mt-3"/>
                <label htmlFor="lineNumber" className='font-poppins text-red-400 px-2 font-bold'>LineNumber</label>
                </div>
                <div className="colors w-1/6 mt-2">
                <Select onChange={this.setLanguage.bind(this)} options={colors}/>
                </div>
                <div className="flex font-poppins text-gray-50">
                  <label htmlFor="backgroundColor" className="font-poppins text-lg mt-4 text-red-400">Background</label>
                  <input type="color" name="backgroundColor" id="bg-color" className="sm:w-24 sm:h-10 ml-2 mt-2 border-collapse border-0" onChange={this.setBackgroud}/>
                </div>
                <div className="last">
                  <button className=' bg-red-500 font-poppins text-lg rounded-lg px-8 py-3 border-0' onClick={()=>{this.exportAsImage(document.getElementById('OwnColor'),"text")}}>Download</button>
                </div>
              </div>

              {/* Text Area Here */}
              <div className="inputArea">
                <textarea name="code-share" id="codeString" cols="30" rows="10" className='ml-10 mr-10 px-5 py-5 w-5/6 bg-slate-500 font-poppins transition-all' onInput={this.setCodeString.bind(this)}>
                </textarea>
              </div>

          <div className="basewrapper full flex justify-center px-10 py-20 ">
              <div className="highlighter w-max flex px-32 py-10 rounded-2xl bg-black" id='OwnColor' >
